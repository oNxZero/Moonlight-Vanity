...
class GlobalListener:
    ...
    def get_nice_name(self, code: int) -> str:
        if code == evdev.ecodes.BTN_LEFT:
            return "Left Click"
        if code == evdev.ecodes.BTN_RIGHT:
            return "Right Click"
        if code == evdev.ecodes.BTN_MIDDLE:
            return "Middle Click"
        if code == evdev.ecodes.KEY_RIGHTSHIFT:
            return "Right Shift"
        if code == evdev.ecodes.KEY_LEFTSHIFT:
            return "Left Shift"
    ...
    def _loop(self):
        while not self.stop_event.is_set():
            if not self.devices:
                self.devices = [
                    d for d in [evdev.InputDevice(path) for path in evdev.list_devices()]
                    if "Moonlight HID" not in d.name
                ]
                if not self.devices:
                    time.sleep(0.5)
                    continue
                ...
                for dev in r:
                    for event in dev.read():
                        ...
                        if self.rebind_mode and val == 1:
                            if self.rebind_mode in ['hide', 'trigger_left', 'trigger_right'] and code in [
                                ...
                                if self.mode_trigger == 'toggle':
                                    ...
                                    if val == 1:
                                        if channel == 'left' and not self.holding_left:
                                            ...
                                            if is_mouse_phys:
                                                self.pending_left_mouse = True
                                            else:
                                                self.start_cb('left')
                                        if channel == 'right' and not self.holding_right:
                                            self.holding_right = True
                                            if is_mouse_phys:
                                                self.pending_right_mouse = True
                                            else:
                                                self.start_cb('right')
                                        ...
                                        if channel == 'left' and self.holding_left:
                                            self.holding_left = False
                                            if self.pending_left_mouse:
                                                self.pending_left_mouse = False
                                            else:
                                                self.stop_cb('left')
                                        if channel == 'right' and self.holding_right:
                                            self.holding_right = False
                                            if self.pending_right_mouse:
                                                self.pending_right_mouse = False
                                            else:
                                                self.stop_cb('right')
